...
try:
    ...
    print(f'TACTIFY_CRASH: {type(_startup_err).__name__}: {_startup_err}', flush=True)
    print(_err_tb, flush=True)
    try:
        st.error(f'**App error — screenshot this for support:**  \n\n`{type(_startup_err).__name__}: {_startup_err}`')
        st.code(_err_tb, language='text')
    except Exception:
        pass
